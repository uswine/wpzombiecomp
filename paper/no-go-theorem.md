...
Observational absence is assigned zero evidentiary weight throughout (§12 explains why); the argument rests on dynamics, not on surveys.
...
Silicates melt at 1,400–1,900 K; iron at 1,811 K; tungsten at 3,695 K; graphite sublimes near 3,900 K; the most refractory known carbides fail by ~4,300 K. **At 0.99c: silicates fail for n ≳ 0.3 cm⁻³, iron-class metals for n ≳ 0.7 — i.e., the mean disk medium melts every ordinary rock and structural metal; any region denser than ~20 cm⁻³ exceeds the failure temperature of every solid known.** Equivalently, at n = 1 the thermal wall sits at β ≈ 0.97 for silicates and iron (0.998 tungsten, 0.999 graphite — chain A31): ordinary matter's transport ceiling is below 0.99c on thermal grounds alone. [I, at the stated densities]
...
## 10. The attainable envelope: likely speed limits of the known mechanisms

The case analysis above says what cannot happen. This section inverts it into the constructive result: **for each known mechanism, the speed it *can* likely reach, and the natural or structural limit that binds it there.** Every ceiling below is derived from the mechanism's own physics (verification chains A31–A35), and the binding constraint is named — this is the paper's positive prediction, falsifiable ceiling by ceiling.

### 10.1 Per-mechanism ceilings

| Mechanism | Binding structural/natural limit | Likely ceiling β | Tag |
|---|---|---|---|
| **Gravitational ejection (natural)** — observed | fastest known: S5-HVS1 (a star), interstellar solids | 0.006 (stars), 10⁻⁴ (solids) | measured |
| **Gravitational ejection (natural)** — theoretical tail | tidal limit at deepest survivable bound orbit; c/3 for stars (Guillochon & Loeb), ISCO speed for strength-bound solids; production rate ≈ 0 at the tail | ~0.03 plausible tail; 0.5 (0.7 Kerr) hard per-exchange scale, at vanishing rate | [P] |
| **Chemical / fission rockets** | mass ratio (exponents of 10³–10⁴) | < 0.001 / ~0.02 | [I] |
| **Fusion rocket** | exhaust rapidity β = tanh((w/c)·ln R): w ≈ 0.05–0.1c, tankage caps R ≲ 100 → 0.23–0.43 in principle; **radiator/dry-mass growth caps it far lower** (Semyonov): at 1 kW/kg rejection and 1% waste, a century mission integrates to Δv ≈ 0.07c | **~0.05–0.1** (Daedalus-class 0.12 is the honest optimum) | [I on energetics; E on radiators] |
| **Photon/antimatter rocket** | radiator bound a ≤ P_rej/(f_w·c): at 1 kW/kg, 1% waste → a = 3.3 × 10⁻⁴ m/s², century mission Δv ≈ 0.0035c; even a 10 kW/kg, 0.1%-waste fantasy yields 0.35c | **~0.003–0.03**; ≤ 0.35 at fantasy parameters | [I — the waste-heat inequality is the wall] |
| **Beamed sail** | diffraction + areal-mass + Doppler-band: advocates' own anchors — 0.26c for a gram-scale wafer at 50–70 GW, 0.01c at 100 kg, 10³ km/s at 100 t (Lubin; Kulkarni et al. confirm ~0.2c diffraction cap for grams) | **~0.2–0.26 (grams); ~0.01 (100 kg); ~0.003 (100 t)** | [E at the power scale; I on the scalings] |
| **Mass driver / coilgun** | strength-capped acceleration a = S/ρL: β = 0.1 per 0.1 AU of track at *ultimate* strength (0.32 at 1 AU, 0.77 at 10 AU) | **~0.1 per 0.1-AU machine**; anything higher buys AU-scale track | [E] |
| **ISM transit (any propulsion)** — thermal | T_eq(β, n) vs. material failure: at n = 1 cm⁻³ the wall sits at **β ≈ 0.97 for silicates/iron**, 0.998 for tungsten, 0.999 for graphite; any n ≳ 20 cm⁻³ region kills all solids at 0.99 | **0.97 for ordinary matter at mean density**; refractories pass thermally and die by the next row | [I] |
| **ISM transit — erosion** | dust cratering ~5 mm/pc at n = 1 (0.2c anchor; per-impact energy 30× worse at 0.99c): a 1-m body loses half its thickness in ~100 pc | distance-dependent: multi-100-pc transport of meter-scale bodies capped near where thermal wall already bites | [I on fluence] |

### 10.2 Reading the envelope

Three regimes emerge:

1. **The likely regime (what the universe and near-term engineering actually deliver): β ≲ 0.03 for macroscopic intact matter.** Nature's observed record is 10⁻⁴ c for solids and 0.006c for stars; its plausible tail reaches a few percent of c. Engineered onboard systems are radiator-bound to the same few percent for anything heavier than a wafer, on any mission that finishes in a career. The gram-scale laser wafer at 0.2c is the one engineered outlier, and it is the least "macroscopic" object in the table.
2. **The heroic regime (every [E] lever pulled): β ~ 0.1–0.5.** Daedalus-class fusion at 0.1; a 100-kg sail at 0.01 even with a 70-GW national-scale laser; AU-scale mass drivers at 0.1–0.3; the gravitational per-exchange scale at 0.5 for a body that survives an SMBH ISCO and wins a lottery with no tickets. Every entry here is an artifact- or rarity-priced ceiling, not a physics wall — and each mechanism's *own structural limit* (radiators, diffraction, track strength, tides) is what stops it, well short of the environmental wall.
3. **The wall (independent of propulsion): β ≈ 0.97 at mean interstellar density for ordinary matter.** Above it, thermal equilibrium exceeds silicate and iron failure temperatures; only curated refractory monoliths pass, into erosion that removes millimeters per parsec and dense regions that kill everything. This is the *natural structural limit of the transport medium itself* — the ceiling that binds even a mechanism-agnostic miracle drive carrying ordinary condensed matter.

**The envelope's summary sentence: known mechanisms plateau one to two orders of magnitude below the medium's own wall (~0.97c), and both plateaus sit far below 0.99c — which is why the no-go of §§4–8 holds with margin: β = 0.99 is above every mechanism's structural ceiling *and* above the transport medium's material ceiling for ordinary solids.** The gap between "likely" (0.03) and "wall" (0.97) is the honest playground for speculation; nothing in it changes the conclusion at 0.99.
...
## 12. Discussion: nature's record, read correctly
...
A31. ISM thermal ceiling: solve T_eq(beta;n)=T_lim (2-face):
     silicate/iron 1500K: beta=0.971 (n=1), 0.996 (n=0.1)
     tungsten 3000K: 0.998 (n=1); graphite 3500K: 0.999 (n=1)
A32. Radiator-limited rocket, century mission (a=P_rej/(f_w c) photon;
     a=2P_rej/(f_w w) massive exhaust):
     200 W/kg @5% waste: photon dv=1e-4 c; fusion(w=0.1c) dv=0.003c
     1 kW/kg @1%: photon 0.0035c; fusion 0.070c
     10 kW/kg @0.1% (fantasy): photon 0.35c
A33. Fusion exhaust bound beta=tanh((w/c)lnR):
     w=0.05c: R=100 -> 0.226; w=0.1c: R=100 -> 0.431, R=1000 -> 0.598
A34. Mass driver at ultimate strength (a=3.3e4 m/s2, 1-m body):
     0.1 AU -> 0.105c; 1 AU -> 0.32c; 10 AU -> 0.77c; 100 AU -> 0.99c
A35. Dust erosion from Hoang 2017 anchor (0.5mm per 3e17 cm^-2 at 0.2c):
     = 5.1 mm/pc at n=1 -> 1-m body loses half its thickness in ~100 pc
```
